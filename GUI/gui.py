...
def appwindow():
    ...
    Label(gui,font=("Ariel",17),text="Product name").place(x=10,y=150)
    exp_itemname = StringVar()
    product_name = Entry(gui,font=("Ariel",15),textvariable=exp_itemname)
    product_name.place(x=220,y=155,height=27,width=165)
    Label(gui,font=("Ariel",17),text="Date(dd-mm-yyyy)").place(x=10,y=200)
    purchase_date = StringVar()
    product_date = Entry(gui,font=("Ariel",15),textvariable=purchase_date)
    product_date.place(x=220,y=205,height=27,width=165)
    Label(gui,font=("Ariel",17),text="Cost of product").place(x=10,y=250)
    purchase_cost = StringVar()
    Label(gui,font=("Ariel",17),text="Select ID to delete").place(x=520,y=170)
    product_cost = Entry(gui,font=("Ariel",15),textvariable=purchase_cost)
    ...
    item_id=StringVar()
    spinbox = Spinbox(gui, font=("Ariel",17),from_= 0, to_ = 200,textvariable=item_id,justify=CENTER)
    spinbox.place(x=745,y=174,height=30,width=50)
    list = Listbox(gui,height=7,width=30,font=("Ariel",20),yscrollcommand = scroll_bar.set)
    list.place(x=168,y=410)
    scroll_bar.config( command = list.yview )
    Button(gui,text="Add Item",font=("Ariel",17),width=10,command=insertitems).place(x=30,y=300)
    Button(gui,text="View all items",font=("Ariel",17), width=12,command=viewallitems).place(x=110,y=355)
    Button(gui,text="Delete all items",font=("Ariel",17),width=15,command=deletealldata).place(x=550,y=280)
    Button(gui,text="Search",font=("Ariel",17),width=10,command=search_item).place(x=220,y=298)
...
Label(root, font=("Ariel", 19), text="Username").place(x=80, y=230)
Label(root, font=("Ariel", 19), text="Password").place(x=80, y=280)

login_username = StringVar()
log_username = Entry(root, font=("Ariel", 15), textvariable=login_username)
log_username.place(x=205, y=238, height=25, width=165)

login_password = StringVar()
log_password = Entry(root, font=("Ariel", 15), textvariable=login_password, show="*")
log_password.place(x=205, y=287, height=25, width=165)

Button(root, text="Login", font=("Ariel", 19), width=12, command=login).place(x=110, y=360)

# Registration Section 
Label(root, font=("Ariel", 19), text="Name").place(x=653, y=195)
Label(root, font=("Ariel", 19), text="Username").place(x=604, y=243)
Label(root, font=("Ariel", 19), text="Password").place(x=610, y=293)
Label(root, font=("Ariel", 17), text="Confirm password").place(x=532, y=342)

register_name = StringVar()
reg_name = Entry(root, font=("Ariel", 15), textvariable=register_name)
reg_name.place(x=740, y=200, height=25, width=165)

register_username = StringVar()
reg_username = Entry(root, font=("Ariel", 15), textvariable=register_username)
reg_username.place(x=740, y=250, height=25, width=165)

register_password = StringVar()
reg_password = Entry(root, font=("Ariel", 15), textvariable=register_password, show="*")
reg_password.place(x=740, y=300, height=25, width=165)

register_repassword = StringVar()
reg_repassword = Entry(root, font=("Ariel", 15), textvariable=register_repassword, show="*")
reg_repassword.place(x=740, y=350, height=25, width=165)

Button(root, text="Register", font=("Ariel", 19), width=12, command = register).place(x=630, y=400)
Button(root, text="Delete all users", font=("Ariel", 15, "bold"), command=deleteallusers).place(x=130, y=620)
Button(root, text="View all users", font=("Ariel", 15, "bold"),command = viewwindow).place(x=130, y=560)
